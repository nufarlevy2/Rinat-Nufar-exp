...
NUMBER_OF_MSG_INVALID_KEY_TYPED = [];
LEFT = 0;
RIGHT = 1;
DOWN = 2;
ESC = 9;
ONE_PICTURE = 0;
TWO_PICTURES = 1;
NUMBER_OF_BLOCKS_FOR_THE_FIRST_PART = 1;
NUMBER_OF_BLOCKS_FOR_THE_SECOND_PART = 0;
BLOCKS_NR = NUMBER_OF_BLOCKS_FOR_THE_FIRST_PART + NUMBER_OF_BLOCKS_FOR_THE_SECOND_PART;
PATH_TO_PICS = char("resources\pics\");
NUMBER_OF_PICS = 60;
NUMBER_OF_TRIALS_PER_BLOCK = NUMBER_OF_PICS;
LEFT_FROM_CENTER_X = -20;
LEFT_FROM_CENTER_Y = 0;
RIGHT_FROM_CENTER_X = 20;
RIGHT_FROM_CENTER_Y = 0;
...
function runExp()        
    ...
    %7. questions
    runQuestions(questions,trial_overall_i);

    %8. close everything nicely...
    terminateExp();
    ...
    function initializeGear()
        if EYE_TRACKING_METHOD==ENUM_EYE_TRACKING_NO_TRACKING && ~EEG                
            ...
            if LAB_ROOM==ExperimentGuiBuilder.ENUM_LAB1                    
                ...
                IN_FUNC= @() io32(IO_OBJ,IN_PORT);
                OUT_FUNC= @(out_code) io32(IO_OBJ,OUT_PORT,out_code);
            ...
            if EYE_TRACKING_METHOD~=ENUM_EYE_TRACKING_NO_TRACKING
                ...
                status = Eyelink('openfile', [EDF_TEMP_FILE_NAME_PREFIX, num2str(SUBJECT_NUMBER), 'eye.edf']);
                if status ~= 0
    ...
    function [texes,dims]= loadResources()
        [texes , dims]= loadTexesFromFolder(PATH_TO_PICS, window, 'jpg', 0, 0);
    ...
    function initializeDataRecord()
        ...
        for trial_i= 1:sum(TRIALS_NR_PER_BLOCK_VEC)
    ...
    function [conditions_mat]= randomizeConditions()
        rng('shuffle');
        TOTAL_TRIALS_NR = BLOCKS_NR * NUMBER_OF_TRIALS_PER_BLOCK;
        conditions_mat = NaN(TOTAL_TRIALS_NR, 2);
        for trial_i1 = 1 : (NUMBER_OF_TRIALS_PER_BLOCK * NUMBER_OF_BLOCKS_FOR_THE_FIRST_PART)
            %random for the first part of the expirament and save it to the
            %condition matrix
            random_numbers = randperm(NUMBER_OF_PICS);
            for ranNum = random_numbers
                conditions_mat(trial_i1,1) = ranNum;
            end
        end
        end
        for trial_i2 = ((NUMBER_OF_TRIALS_PER_BLOCK * NUMBER_OF_BLOCKS_FOR_THE_FIRST_PART)+1) : TOTAL_TRIALS_NR
            %random for the second part of the exp and save it to the
            %condition matrix
        end
        % randi, randperm(pictures_number,2)
    ...
    function trial_overall_i = runBlocks(conditions_mat)
        ...
        for block_i= 1:BLOCKS_NR     
            if block_i <= NUMBER_OF_BLOCKS_FOR_THE_FIRST_PART
                option = ONE_PICTURE;
            elseif (block_i > NUMBER_OF_BLOCKS_FOR_THE_FIRST_PART && block_i <= NUMBER_OF_BLOCKS_FOR_THE_SECOND_PART)
                option = TWO_PICTURES;
            end
            runCalibrationQuery(); 
            ...
            for trial_inside_block_i= 1:trials_nr_on_curr_block                                       
                ...
                trial_vector= [conditions_mat(trial_overall_i,:),option];                                                 
                [trial_start_vbl, trial_end_vbl] = runTrial(trial_vector);
                ...
                if (~IS_EXP_GO)
                    ...
                    EXPDATA.info.general_info.experiment_net_duration= trial_end_vbl - exp_start_vbl - total_experiment_pauses_duration;
                    break;
                end
        ...
        exp_end_vbl= drawImageInstructions(msgs{3});    
        ...
        % ---------------------------------------------------------------------------------------------------
        function [trial_start_vbl, trial_end_vbl, subject_respones]= runTrial(trial_params)                           
            sendTrigger(TRIGGERS_START_TRAIL);
            drawFixationCross(FIXATION_CROSS_ARMS_LEN, FIXATION_CROSS_ARMS_WIDTH, FIXATION_CROSS_COLOR);
            if trial_params(3) == ONE_PICTURE
                picCenter = trail_params(1);
                Screen('DrawTexture', window, texes(picCenter), genRect([0,0], [dims(picCenter,1),dims(picCenter,2)]));
                EXPDATA.trials(trial_overall_i).part = 1;
                EXPDATA.trials(trial_overall_i).left_picture = trial_params{1};
            else
                picLeft = trial_params{1};
                picRight = trial_params{2};
                Screen('DrawTexture', window, texes(picLeft), genRect([LEFT_FROM_CENTER_X, LEFT_FROM_CENTER_Y], [dims(picLeft,1),dims(picLeft,2)]));
                Screen('DrawTexture', window, texes(picRight), genRect([RIGHT_FROM_CENTER_X, RIGHT_FROM_CENTER_Y], [dims(picRight,1),dims(picRight,2)]));
                EXPDATA.trials(trial_overall_i).part = 2;
                EXPDATA.trials(trial_overall_i).left_picture = trial_params{1};
            ...
            % if this is the first part of the expirement, the instructions
            % that needs to be displayed is that:
            % Right = Yes
            % Left = No
            % Down = Abstain
            while (1)
    ...
    function terminateExp()
        ...
        if EYE_TRACKING_METHOD~=ENUM_EYE_TRACKING_NO_TRACKING
            ...
            Eyelink('CloseFile');
            stat = Eyelink('ReceiveFile',[EDF_TEMP_FILE_NAME_PREFIX, num2str(SUBJECT_NUMBER), 'eye.edf']);
            if stat ~= 0
            ...
            movefile([EDF_TEMP_FILE_NAME_PREFIX, num2str(SUBJECT_NUMBER), 'eye.edf'], fullfile(EDF_SAVE_FOLDER, [EDF_FILE_NAME_PREFIX, num2str(SUBJECT_NUMBER), EDF_FILE_NAME_SUFFIX, '.edf']));                
            Eyelink('Shutdown');
        end 
