...
classdef Eyeballer < handle
    ...
    properties (Access= private, Constant)
        ...
        SACCADES_COLORS= [[0, 209, 24]/255; [255, 0, 0]/255; [0, 0, 255]/255];
    end
    ...
    methods (Access= public)
        function obj= Eyeballer(save_func, raw_eye_data, timeline_left_offset, sampling_rates, manual_saccade_search_params, saccades_data, main_fig_pos, main_gui_background_color)   
            ...
            for subject_i= 1:subjects_nr
                ...
                % obj.eye_data{subject_i}(trial_i).left_x, left_y, right_x, right_y
            ...
            for subject_i= 1:subjects_nr
                for trial_i = 1:numel(obj.eye_data{subject_i})
                    data_ranges = [obj.mean_range, max([abs(obj.eye_data{subject_i}(trial_i).left_x), ...
            ...
            image('cdata', legend_img_cdata(end:-1:1, :, 1:3),'alphadata', legend_img_cdata(end:-1:1,:, 4));
            obj.eyes_x_coords_axes= axes('Box', 'on', 'position', [0.05, 0.05, 0.75, 0.38], 'ButtonDownFcn', @obj.eyeCoordsAxesButtonDownCallback);
            obj.eyes_y_coords_axes= axes('Box', 'on', 'position', [0.05, 0.53, 0.75, 0.38], 'ButtonDownFcn', @obj.eyeCoordsAxesButtonDownCallback);
            ...
            uicontrol(interface_panel, 'Style', 'pushbutton', 'units', 'normalized', 'tag', 'c2000', ...
                ...
                'Position', [0.0740    0.2263    0.8420    0.0509], ...
                'FontSize', 10.0, ...
                'BackgroundColor', obj.main_gui_background_color, ....
        ...
        function [saccades_struct, eyeballing_session_stats]= getSaccadesStruct(obj)
            ...
            if obj.is_eyeballing_accepted                 
                ...
                for subject_i= 1:subjects_nr                                                           
                    ...
                    for cond_i= 1:curr_subject_conds_nr 
                        saccades_struct{subject_i}.(curr_subject_conds_names{cond_i}) = [];                    
                        curr_subject_cond_trials_nr= numel( obj.original_saccades_data{subject_i}.(curr_subject_conds_names{cond_i}) );
                        eyeballing_session_stats{subject_i}.(curr_subject_conds_names{cond_i}).accepted_microsaccades_nr= zeros(curr_subject_cond_trials_nr, 1);
                        eyeballing_session_stats{subject_i}.(curr_subject_conds_names{cond_i}).rejected_microsaccades_nr= zeros(curr_subject_cond_trials_nr, 1);
                        eyeballing_session_stats{subject_i}.(curr_subject_conds_names{cond_i}).user_generated_nr= zeros(curr_subject_cond_trials_nr, 1);
                        for trial_i= 1:curr_subject_cond_trials_nr                            
                            ...
                            if ~isempty( eyeballing_altered_saccades_data_unshuffled(prev_conds_trials_nr + trial_i).user_codes )
                                ...
                                eyeballing_session_stats{subject_i}.(curr_subject_conds_names{cond_i}).rejected_microsaccades_nr(trial_i)= ...
                                    sum( eyeballing_altered_saccades_data_unshuffled(prev_conds_trials_nr + trial_i).user_codes == obj.ENUM_REJECTED_SACCADE_CODE );
                                eyeballing_session_stats{subject_i}.(curr_subject_conds_names{cond_i}).user_generated_nr(trial_i)= ...
    ...
    methods (Access= private)                        
        function plotCurrTrialSaccades(obj)   
            ...
            times_vec= find(curr_subject_trial_eye_data_struct.non_nan_times_logical_vec); 
            
            if isempty(times_vec)
                [no_data_on_x_plot, no_data_on_y_plot] = plotBlinkSeg(1:trial_dur);                
            else
                no_data_on_x_plot = [];
                segs_end_times_is= find(diff(times_vec)>1); 
                segs_start_times= [times_vec(1), times_vec(segs_end_times_is+1)];
                segs_end_times= [times_vec(segs_end_times_is), times_vec(end)];            
                if times_vec(1)>1
                    [no_data_on_x_plot, no_data_on_y_plot] = plotBlinkSeg(1:times_vec(1)-1);
                end

                for seg_i= 1:numel(segs_end_times)
                    eye_data_seg_times= segs_start_times(seg_i):segs_end_times(seg_i);
                    [x_ax_left_eye_plot, x_ax_right_eye_plot, y_ax_left_eye_plot, y_ax_right_eye_plot] = ...
                        plotEyeDataSeg(eye_data_seg_times);
                    if seg_i<numel(segs_end_times)
                        blink_seg_times= segs_end_times(seg_i)+1:segs_start_times(seg_i+1)-1; 
                        [no_data_on_x_plot, no_data_on_y_plot] = plotBlinkSeg(blink_seg_times);                        
                    end                           
                end                        

                if times_vec(end)<trial_dur
                    [no_data_on_x_plot, no_data_on_y_plot] = plotBlinkSeg(times_vec(end)+1:trial_dur);
                end
                ...
                for saccade_i= 1:curr_subject_trial_saccades_nr
                    ...
                    saccade_times= times_vec(find(times_vec==curr_saccade_onset):find(times_vec==curr_saccade_offset));                                
            ...
            set(obj.eyes_x_coords_axes, 'YLim', 7*obj.std_range + [-obj.mean_range, obj.mean_range]);
            set(obj.eyes_y_coords_axes, 'YLim', 7*obj.std_range + [-obj.mean_range, obj.mean_range]);        
            set(obj.fig, 'name', obj.FIG_TITLE);   
            disp('Done.');
            
            function [plot_x_ax_left_eye_plot, plot_x_ax_right_eye_plot, plot_y_ax_left_eye_plot, plot_y_ax_right_eye_plot] = plotEyeDataSeg(times)        
                plot_x_ax_left_eye_plot = plot(obj.eyes_x_coords_axes, times - obj.timeline_left_offset, curr_subject_trial_eye_data_struct.left_x(times), 'color', obj.LEFT_EYE_PLOT_COLOR, 'ButtonDownFcn', @obj.eyeDataPlotBtnDownCallback);
                plot_x_ax_right_eye_plot = plot(obj.eyes_x_coords_axes, times - obj.timeline_left_offset, curr_subject_trial_eye_data_struct.right_x(times), 'color', obj.RIGHT_EYE_PLOT_COLOR, 'ButtonDownFcn', @obj.eyeDataPlotBtnDownCallback);            
                plot_y_ax_left_eye_plot = plot(obj.eyes_y_coords_axes, times - obj.timeline_left_offset, curr_subject_trial_eye_data_struct.left_y(times), 'color', obj.LEFT_EYE_PLOT_COLOR, 'ButtonDownFcn', @obj.eyeDataPlotBtnDownCallback);                            
                plot_y_ax_right_eye_plot = plot(obj.eyes_y_coords_axes, times - obj.timeline_left_offset, curr_subject_trial_eye_data_struct.right_y(times), 'color', obj.RIGHT_EYE_PLOT_COLOR, 'ButtonDownFcn', @obj.eyeDataPlotBtnDownCallback);                           
            end
        ...
        function saccade_plots_hs= plotSaccade(obj, saccade_times, plot_color)
            saccade_times= saccade_times(obj.eye_data{obj.curr_subject}(obj.curr_trial).non_nan_times_logical_vec(saccade_times));
            curr_subject_trial_eye_data_struct= obj.eye_data{obj.curr_subject}(obj.curr_trial);
        ...
        function eyeCoordsAxesButtonDownCallback(obj, ~, ~)
            disp('air hit.');
%             point_on_axis= get(obj.eye_coords_axes, 'CurrentPoint');
%             curr_mouse_pos= point_on_axis(1,1:2);
%             curr_subject_trial_eye_data_struct= obj.eye_data{obj.curr_subject}(obj.curr_trial);
%             if ( abs(curr_subject_trial_eye_data_struct.left_y(floor(curr_mouse_pos(1)))-curr_mouse_pos(2))<2*obj.SACCADE_MARKERS_SIZE || ...
%                  abs(curr_subject_trial_eye_data_struct.right_y(floor(curr_mouse_pos(1)))-curr_mouse_pos(2))<2*obj.SACCADE_MARKERS_SIZE )
%                 obj.eyeDataPlotBtnDownCallback();
%             end
        end
        
        function eyeDataPlotBtnDownCallback(obj, ~, ~)
            obj.user_redo_stack= {};
            point_on_axis= get(gca, 'CurrentPoint');
            curr_mouse_pos_x= floor(point_on_axis(1));            
...
%             save('zoom_out_icon_cdata.mat', 'zoom_out_icon_cdata');            
        ...
        function cleared_data= clearSaccadeData(obj, saccade_i)
            saccade_parameters_names= fieldnames(obj.eyeballing_altered_saccades_data{obj.curr_subject}(obj.curr_trial));
            cleared_data= zeros(1, numel(saccade_parameters_names));
        ...
        function mouseMovedCallback(obj, ~, ~)
            ...
            curr_axes_point_x= floor(curr_axes_point(1,1));
            curr_axes_point_y= curr_axes_point(1,2);
            curr_subject_trial_eye_data_struct= obj.eye_data{obj.curr_subject}(obj.curr_trial);
            epsilon= 0.18*diff(get(curr_axes, 'YLim'));
            if 1<=curr_axes_point_x && curr_axes_point_x<=numel(curr_subject_trial_eye_data_struct.left_x) && isCursorCloseToPlot()
                ...
                if ~isempty(pointed_saccade_i)
                    ...
                    primal_text_x_pos= curr_axes_point_x+0.25*epsilon;
